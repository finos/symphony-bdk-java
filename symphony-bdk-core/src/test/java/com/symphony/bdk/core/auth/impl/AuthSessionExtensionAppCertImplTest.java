...
public class AuthSessionExtensionAppCertImplTest {
  ...
  void testRefresh() throws AuthUnauthorizedException {
    ...
    final ExtensionAppAuthenticatorCertImpl auth = mock(ExtensionAppAuthenticatorCertImpl.class);
    when(auth.retrieveExtensionAppSession(appToken)).thenReturn(appTokens);

    final AppAuthSession session = new AppAuthSessionCertImpl(auth, appToken);
    session.refresh();

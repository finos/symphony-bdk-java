...
public class DatafeedLoopV1 extends AbstractDatafeedLoop {
  ...
  public DatafeedLoopV1(DatafeedApi datafeedApi, AuthSession authSession, BdkConfig config) {
    this(datafeedApi, authSession, config, config.getDatafeed().isReuseDatafeedId() ?
        new OnDiskDatafeedIdRepository(config) : new InMemoryDatafeedIdRepository(config.getAgent().getBasePath()));
  }

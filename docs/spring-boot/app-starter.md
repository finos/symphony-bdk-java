...
- Application Authenticate: `POST /bdk/v1/app/auth`:
...
- Validate tokens: `POST /bdk/v1/app/tokens`:

    Parameters:
    ```json
    {
      "appToken": "your-app-token",
      "symphonyToken": "symphony-token" // Symphony token returned from Symphony App Frontend
    }
    ``` 
  
- Validate jwt: `POST /bdk/v1/app/tokens`

    Parameters:
    ```json
    {
      "jwt": "your-jwt" // Jwt signed by Symphony App Frontend
    }
    ``` 
